...
module.exports.getActeur = function(id,callback){
    db.getConnection(function (err,connexion) {
        if(!err){
            let sql ="SELECT ACTEUR_DATEDEBUT as date_debut, ROLE_NOM as role, FILM_TITRE as film_titre, FILM_DATEREALISATION as film_date FROM vip as v JOIN acteur as a ON v.VIP_NUMERO=a.VIP_NUMERO LEFT JOIN joue as j ON a.VIP_NUMERO=j.VIP_NUMERO JOIN film as f ON j.FILM_NUMERO=f.FILM_NUMERO WHERE a.VIP_NUMERO = '"+id+"'";
            //console.log(sql);
            connexion.query(sql, callback);
            connexion.release();
        }
